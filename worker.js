...
export default {
  ...
  const postData = await env.BLOG_KV.get(postKey, 'json');
  if (!postData) {
  return safeJsonResponse({ error: ' 帖子不存在 ' }, 404);
  }
  //  增加阅读量
  postData.views = (postData.views || 0) + 1;
  await env.BLOG_KV.put(postKey, JSON.stringify(postData));
  return safeJsonResponse({ views: postData.views });
  } catch (e) {
  console.error(' 增加阅读量时出错 :', e);
  return safeJsonResponse({ error: ' 无法增加阅读量 ' }, 500);
  }
  }
  //  删除帖子（仅管理员）
...
<body>
  ...
  <div>本站已运行: <span id="runtime">0天0小时0分0秒</span></div>
  </footer>
  <script>
  //  全局状态
  const state = {
  ...
  title: localStorage.getItem('title') || '注册会员',  //  新增头衔状态
  ...
  runtime: document.getElementById('runtime')  //  新增运行时间元素
  };
  //  显示错误
  ...
  if (diff < 0) {
  elements.runtime.textContent = '尚未开始';
  return;
  }
  ...
  state.title = data.title || '注册会员';  //  保存头衔
  ...
  localStorage.setItem('title', data.title || '注册会员');  //  保存头衔
  ...
  var safeTitleText = escapeHTML(post.title || '');
  var titleClass = post.title === '创始人' ? 'title-founder' : 'title-member';
  ...
  '<span class="title ' + titleClass + '">' + safeTitleText + '</span>' +
  '</div>' +
  '<div class="post-time">' + new Date(post.createdAt).toLocaleString() + '</div>' +
  '</div>' +
  '</div>' +
  '<div class="post-meta">阅读量: ' + (post.views || 0) + '</div>' +
  ...
  if (postElement) {
  var viewElement = postElement.querySelector('.post-meta');
  if (viewElement) {
  viewElement.textContent = '阅读量: ' + data.views;
  }
  }
  }
  })
  ...
  var titleClass = comment.title === '创始人' ? 'title-founder' : 'title-member';
  var safeTitleText = escapeHTML(comment.title || '');
  ...
  var titleClass = state.title === '创始人' ? 'title-founder' : 'title-member';
  ...
  '<div>' +
  '<div>' + state.nickname + 
  '<span class="title ' + titleClass + '">' + escapeHTML(state.title) + '</span>' +
  ' ' + (state.role === 'admin' ? '( 管理员 )' : '') + '</div>' +
  ...
  //  显示头衔
  var titleClass = user.title === '创始人' ? 'title-founder' : 'title-member';
  var titleHtml = user.title ? '<span class="title ' + titleClass + '">' + escapeHTML(user.title) + '</span>' : '';
  elements.profileNickname.innerHTML = escapeHTML(user.nickname) + titleHtml;
  ...
  localStorage.removeItem('title');  //  删除头衔
