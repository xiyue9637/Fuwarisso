...
export default {
  ...
  // 设置用户角色（仅管理员）
  if (pathname === '/api/set-role' && request.method === 'POST') {
  ...
  const { username, role, title } = data;
  if (!username || !role) {
  return safeJsonResponse({ error: ' 需要提供用户名和角色 ' }, 400);
  }
  ...
  // 如果之前是管理员，现在降级为普通用户，可以考虑保留原有头衔或设置为注册会员
  if (!userObj.title || userObj.title === ' 管理员 ') {
  userObj.title = ' 注册会员 ';
  }
  }
  ...
  console.error(' 设置角色时出错 :', e);
  return safeJsonResponse({ error: ' 无法设置角色 ' }, 500);
  }
  }
  // 创建新头衔（仅管理员）
  ...
  if (!name || !style) {
  return safeJsonResponse({ error: ' 需要提供头衔名称和样式 ' }, 400);
  }
  try {
  const titleId = generateUUID();
  await env.BLOG_DATA_STORE.put(`titles/${titleId}`, JSON.stringify({
  id: titleId,
  name,
  style,
  createdAt: new Date().toISOString()
  ...
  // 为用户设置头衔（仅管理员）
  if (pathname.startsWith('/api/users/') && pathname.endsWith('/title') && request.method === 'PUT') {
  const { valid, error, user: currentUser } = await checkPermission(env, request);
  if (!valid) return safeJsonResponse({ error }, 403);
  // 只有管理员可以设置头衔
  ...
  const { title } = data;
  if (!title) {
  return safeJsonResponse({ error: ' 需要提供头衔 ' }, 400);
  }
  try {
  const userKey = `users/${username}`;
  const userData = await env.BLOG_DATA_STORE.get(userKey);
  ...
  console.error(' 设置头衔时出错 :', e);
  return safeJsonResponse({ error: ' 无法设置头衔 ' }, 500);
  }
  }
  // 获取所有帖子
  ...
  // 增加帖子阅读数量
  if (pathname.startsWith('/api/posts/') && pathname.endsWith('/view') && request.method === 'POST') {
  const { valid, error, user } = await checkPermission(env, request);
  if (!valid) return safeJsonResponse({ error }, 403);
  ...
  const postData = await env.BLOG_DATA_STORE.get(postKey);
  if (!postData) {
  ...
  return safeJsonResponse({ error: ' 帖子数据损坏 ' }, 500);
  }
  
  // 检查用户是否已经阅读过
  const viewedKey = `post-views/${postId}/${request.headers.get('cf-connecting-ip') || 'unknown'}`;
  const hasViewed = await env.BLOG_DATA_STORE.get(viewedKey);
  
  if (!hasViewed) {
  // 增加阅读数量
  post.views = (post.views || 0) + 1;
  await env.BLOG_DATA_STORE.put(postKey, JSON.stringify(post));
  // 记录已阅读
  await env.BLOG_DATA_STORE.put(viewedKey, '1', { expirationTtl: 86400 }); // 24小时
  }
  
  return safeJsonResponse({ views: post.views });
  } catch (e) {
  console.error(' 增加阅读数量时出错 :', e);
  return safeJsonResponse({ error: ' 无法增加阅读数量 ' }, 500);
  }
  }
  // 点赞帖子
  ...
  // 检查用户是否已经点赞
  const likeKey = `post-likes/${postId}/${user.username}`;
  const hasLiked = await env.BLOG_DATA_STORE.get(likeKey);
  
  if (hasLiked) {
  // 取消点赞
  post.likes = Math.max(0, (post.likes || 0) - 1);
  await env.BLOG_DATA_STORE.delete(likeKey);
  } else {
  // 点赞
  post.likes = (post.likes || 0) + 1;
  await env.BLOG_DATA_STORE.put(likeKey, '1');
  }
  
  await env.BLOG_DATA_STORE.put(postKey, JSON.stringify(post));
  return safeJsonResponse({ likes: post.likes, liked: !hasLiked });
  } catch (e) {
  console.error(' 点赞帖子时出错 :', e);
  return safeJsonResponse({ error: ' 无法点赞 ' }, 500);
  }
  }
  // 点赞评论
  if (pathname.startsWith('/api/comments/') && pathname.endsWith('/like') && request.method === 'POST') {
  const { valid, error, user } = await checkPermission(env, request);
  if (!valid) return safeJsonResponse({ error }, 403);
  ...
  try {
  const commentKey = `comments/${postId}/${commentId}`;
  const commentData = await env.BLOG_DATA_STORE.get(commentKey);
  if (!commentData) {
  return safeJsonResponse({ error: ' 评论不存在 ' }, 404);
  }
  ...
  return safeJsonResponse({ error: ' 评论数据损坏 ' }, 500);
  }
  
  // 检查用户是否已经点赞
  const likeKey = `comment-likes/${postId}/${commentId}/${user.username}`;
  const hasLiked = await env.BLOG_DATA_STORE.get(likeKey);
  ...
  await env.BLOG_DATA_STORE.put(commentKey, JSON.stringify(comment));
  return safeJsonResponse({ likes: comment.likes, liked: !hasLiked });
  } catch (e) {
  console.error(' 点赞评论时出错 :', e);
  return safeJsonResponse({ error: ' 无法点赞 ' }, 500);
  }
  }
...
<head>
  ...
  * {
  ...
  .title-founder {
  background: linear-gradient(to right, #d40000, #a50000);
  color: gold;
  padding: 1px 5px;
  border-radius: 3px;
  font-weight: bold;
...
<body>
  ...
  <footer>
  本站已稳定运行 <span id="runtime">0天 00:00:00</span>
  </footer>
  <script>
  // 全局状态
  const state = {
  ...
  if (diff < 0) {
  elements.runtime.textContent = '0天 00:00:00';
  return;
  }
  ...
  \`\${days}天 \${remainingHours.toString().padStart(2, '0')}:\${remainingMinutes.toString().padStart(2, '0')}:\${remainingSeconds.toString().padStart(2, '0')}\`;
  }
  ...
  var titleHTML = '';
  if (post.title) {
  if (post.title.trim() === '创始人') {
  titleHTML = '<span class="title-founder">' + escapeHTML(post.title) + '</span>';
  } else if (post.title.trim() === '管理员') {
  titleHTML = '<span class="title-admin">' + escapeHTML(post.title) + '</span>';
  } else if (post.title.trim() === '注册会员') {
  titleHTML = '<span class="title-member">' + escapeHTML(post.title) + '</span>';
  } else {
  titleHTML = '<span class="title-custom">' + escapeHTML(post.title) + '</span>';
  }
  }
  ...
  '<div>' +
  '<div class="author">' + safeNickname + ' ' + titleHTML + '</div>' +
  '<div class="post-meta">阅读 ' + (post.views || 0) + ' · ' + new Date(post.createdAt).toLocaleString() + '</div>' +
  '</div>' +
  '</div>' +
  '<h3 class="post-title">' + safeTitle + '</h3>' +
  '<div class="post-content">' + safeContent + '</div>';
  // 添加点赞按钮
  html += '<div class="post-actions" style="display: flex; gap: 15px; margin-top: 10px; color: #777; font-size: 0.9rem;">' +
  '<button class="like-btn" data-post-id="' + escapeHTML(post.id) + '" ' +
  ...
  var titleHTML = '';
  if (comment.title) {
  if (comment.title.trim() === '创始人') {
  titleHTML = '<span class="title-founder">' + escapeHTML(comment.title) + '</span>';
  } else if (comment.title.trim() === '管理员') {
  titleHTML = '<span class="title-admin">' + escapeHTML(comment.title) + '</span>';
  } else if (comment.title.trim() === '注册会员') {
  titleHTML = '<span class="title-member">' + escapeHTML(comment.title) + '</span>';
  } else {
  titleHTML = '<span class="title-custom">' + escapeHTML(comment.title) + '</span>';
  }
  }
  ...
  '<span class="comment-author">' + safeNickname + ' ' + titleHTML + '</span>' +
  '<span class="comment-time">' + new Date(comment.createdAt).toLocaleString() + '</span>' +
  '</div>' +
  '<p>' + safeContent + '</p>';
  // 添加点赞按钮
  html += '<div class="comment-actions" style="display: flex; gap: 10px; margin-top: 5px; color: #777; font-size: 0.8rem;">' +
  '<button class="like-btn" data-comment-id="' + escapeHTML(comment.id) + '" ' +
  ...
  // 添加评论点赞事件
  var commentLikeButtons = commentsList.querySelectorAll('.like-btn');
  for (var i = 0; i < commentLikeButtons.length; i++) {
  commentLikeButtons[i].addEventListener('click', function(e) {
  e.stopPropagation();
  var commentId = this.getAttribute('data-comment-id');
  var postId = this.getAttribute('data-post-id');
  if (commentId && postId) {
  // 评论点赞
  fetch('/api/comments/' + postId + '/' + commentId + '/like', {
  method: 'POST',
  headers: { 'Authorization': 'Bearer ' + state.token }
  })
  .then(function(response) {
  ...
  var titleHTML = '';
  if (state.title) {
  if (state.title.trim() === '创始人') {
  titleHTML = '<span class="title-founder">' + escapeHTML(state.title) + '</span>';
  } else if (state.title.trim() === '管理员') {
  titleHTML = '<span class="title-admin">' + escapeHTML(state.title) + '</span>';
  } else if (state.title.trim() === '注册会员') {
  titleHTML = '<span class="title-member">' + escapeHTML(state.title) + '</span>';
  } else {
  titleHTML = '<span class="title-custom">' + escapeHTML(state.title) + '</span>';
  }
  }
  ...
  '<div>' +
  '<div>' + state.nickname + ' ' + titleHTML + ' ' + (state.role === 'admin' ? '(管理员)' : '') + '</div>' +
  ...
  if (user.title) {
  if (user.title.trim() === '创始人') {
  titleElement.className = 'title-founder';
  } else if (user.title.trim() === '管理员') {
  titleElement.className = 'title-admin';
  } else if (user.title.trim() === '注册会员') {
  titleElement.className = 'title-member';
  } else {
  titleElement.className = 'title-custom';
  }
  titleElement.textContent = user.title;
  elements.profileNickname.appendChild(titleElement);
  }
  // 检查是否是当前用户
